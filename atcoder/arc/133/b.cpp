...
int main(){
    ...
    rep(i, 1, n){
        vector<ll> v;
        rep(j, 1, n / p[i]){
            v.push_back(iq[j * p[i]]);
        }
        sort(v.begin(), v.end(), greater<>());

        zep(j, 0, v.size()){
            sg.range_maxupdate(v[j], v[j] + 1, sg.query_max(0, v[j]) + 1);
            // cout << p[i] << " " << v[j] << " " << (*mp.upper_bound(-v[j])).second + 1 << endl;
        }
    }

    print(sg.query_max(0, n + 1))
